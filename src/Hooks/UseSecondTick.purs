...
type UseSecondTick = UseState (Maybe String) <> UseEffect <> Hooks.Pure

useSecondTick
  :: forall m
   . MonadAff m
  => Hook m UseSecondTick (Maybe String)
useSecondTick = Hooks.do
  ...
  Hooks.pure tStr -- [4]
  where
    subscribeToTimer :: ((Maybe String) -> HookM m Unit) -> HookM m H.SubscriptionId
    subscribeToTimer putTime = do

      let 
        putTime' :: forall a . a -> HookM m Unit
        putTime' =  putTime <<< Just <=< liftEffect <<< (map show) <<< (const EffNow.now)
      t <- timer (putTime' unit)
